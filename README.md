...
### Private Gmail → Alaska Airlines conformance

On the exact `https://www.alaskaair.com/booking/reservation-lookup` route, Gmail mode switches to a dedicated 13-month query for `Your flight is booked` mail from `reservation@email.alaskaair.com`. ContextFill requires Alaska domain evidence, an explicitly labeled confirmation code, and exactly one two-part traveler name in the labeled `Traveler(s)` section. It does not reuse the generic 15-minute code scan or infer surname from a greeting.

After masked booking selection, the in-page trace maps only passenger last name and confirmation code, transfers them atomically, never presses **Continue**, and offers whole-handoff Undo. Multiple travelers, compound-name ambiguity, spoofed senders, unrelated messages, route changes, and replay block. Completed reservations may still be archived by the airline; this path verifies ContextFill's handoff rather than downstream reservation availability.
