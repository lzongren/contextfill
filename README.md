...
The same deterministic message-to-page engine also supports **Verified Magic-Link Handoff**, verification codes, and one narrowly scoped **Trusted Reference Transfer**. It never automatically navigates, copies a value, submits a form, persists capsule facts, or lets a model authorize an action.
...
flowchart LR
  ...
  C["Current origin and DOM"] --> P["Deterministic trust policy"]
  X --> P
  P --> M["Conservative field map"]
  M --> UI["Masked trust trace and field plan"]
  UI -->|"Explicit transfer"| A["Atomic set and verify"]
  A --> R["Reversible receipt"]
